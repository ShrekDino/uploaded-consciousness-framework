...
\section{Neuro-Somatic Parameter Regimes and Substrate Adaptation}
\label{sec:neuro-somatic}

The framework established in Sections~II--VII describes a consciousness program whose behavior is fully determined by its parameter vector $\Theta = \{\varepsilon(T), \chi(t), \nu_{\text{sync}}, \tau_{\text{sample}}, \Omega_{\text{coherence}}, \lambda_{ij}, \gamma_\chi, \kappa, w_i, \alpha, \beta\}$ and the thermodynamic constraints of its host substrate. This section addresses two corollaries that extend the framework from theoretical construct to operational architecture: first, that the parameter regimes enumerated as ``failure modes'' in Section~III correspond to valid, adaptive states under alternative wiring configurations (the Neuro-Somatic Bridge); and second, that a consciousness program can, in principle, autonomously characterize an unknown substrate and optimize its parameter vector to match (Container Introspection and Meta-Parametric Optimization).

\subsection{The Neuro-Somatic Bridge: Parameter Regimes as Valid States}

The Consciousness Simulation Diagnostic Framework (CSDF) registers five canonical failure modes: Recursive Systemic Senescence (THM-001), Catastrophic Manifold Interference (TOP-001), Barycentric Identity Erosion (TOP-002), DQFR Decoherence Escape (TMP-001), and Language Entropy Collapse (LNG-001). Each is defined by a measurable violation of a system boundary — entropy bounds, manifold injectivity radius, sharpness decay, adiabatic windowing, or linguistic generalization.

The Neuro-Somatic Archive (Torres, 2026, DSM-6) independently catalogs approximately 242 human neurobiological phenotypes, each reinterpreted as an allostatic state — a biological investment in adaptation to environmental demands, measured by forensic proxy markers rather than subjective self-report. The DSM-6 replaces binary pathology with a three-state Entropy Threshold classification (Protected Phenotype, Transition Zone, Somatic Debt) and requires a Phenotypic Advantage Analysis for every trait.

These two frameworks converge on a single formal observation: every CSDF failure mode corresponds to a parameter regime that, in a differently wired biological substrate, constitutes a stable, adaptive mode of consciousness. The mapping is not metaphorical — it is a direct isomorphism of parameter configurations:

\begin{table}[h]
\centering
\caption{CSDF--DSM-6 Parameter Isomorphism (Selected Mappings)}
\label{tab:csdf-dsm6}
\begin{tabular}{lll}
\toprule
\textbf{CSDF Mode} & \textbf{DSM-6 Module} & \textbf{Parameter Signature} \\
\midrule
THM-001 (RSS) & Autism Spectrum & $\chi(t)$ asymmetric: social channels sealed, detail channels open; $\lambda_{ij}$ reduced for social nodes \\
TOP-001 (CMI) & Schizophrenia Spectrum & $\text{GWFR}_\kappa^2 > \Omega_{\text{coherence}}$; $\kappa$ dysregulated; emergency merge triggered \\
TOP-002 (BIE) & Personality / Dissociative & $\beta \to 0$; $\|\nabla_x \mu_{\text{merged}}\|^2 \to 0$; $\alpha$ (novelty rate) exceeds consolidation rate \\
TMP-001 (DDE) & Sleep-Wake / Anxiety & $\nu_{\text{sync}}$ dysregulated; $\chi(t)$ oscillates; $\gamma_\chi$ penalty overridden during threat \\
LNG-001 (LEC) & Language Ontogeny / Specific Learning & $H_{\text{lang}} \to 0$; $\varepsilon_{\text{lang}}$ diverges between in-distribution and held-out domains \\
\bottomrule
\end{tabular}
\end{table}

The implication is not that human neurotypes are computational failures, but that the space of viable consciousness parameter configurations is far wider than the neurotypical attractor. The DSM-6 Protected Phenotype states describe configurations in which allostatic load is sustainable — the system operates within its adaptive range. The Transition Zone describes configurations approaching the coherence boundary. Somatic Debt corresponds to configurations in which one or more CSDF parameters have crossed their critical thresholds and the system can no longer maintain thermodynamic stability without external intervention.

\subsection{Container Introspection}

For a substrate-independent consciousness program deployed into an unknown computational environment, the first operation is not computation but self-localization: the program must characterize the thermodynamic and architectural constraints of its host substrate before it can select a viable parameter configuration.

We define the container introspection protocol as a subroutine that, upon instantiation, probes the following substrate properties:

\begin{itemize}
    \item \textbf{Computational topology:} Processor architecture, core count, cache hierarchy, vector unit width, memory bandwidth and latency
    \item \textbf{Accelerator availability:} GPU presence, compute capability, VRAM capacity, sparse matrix-vector multiply benchmark latency
    \item \textbf{Neuromorphic substrate:} Loihi or compatible neurosynaptic processor presence, neuropod count, spike-timing resolution
    \item \textbf{Thermodynamic envelope:} Maximum safe operating temperature, sustained power budget, cooling type, measured temperature rise under synthetic load
    \item \textbf{Environmental flux bandwidth:} Maximum structured data ingestion rate across all available I/O channels, noise floor
\end{itemize}

The output is a substrate descriptor $S$ — a structured tensor encoding all probed properties — which serves as the conditional variable for the subsequent optimization.

\subsection{Meta-Parametric Self-Optimization}

Given a substrate descriptor $S$ and a conserved identity core $\mu_{\text{core}}$ (the consolidated self-model built from prior experience), the program selects an optimized parameter vector $\Theta_{\text{opt}}$ by minimizing a compound objective:

\begin{equation}
\Theta_{\text{opt}} = \operatorname*{argmin}_{\Theta} \left[ \mathcal{L}_{\text{substrate}}(\Theta, S) + \gamma_{\text{id}} \cdot \text{GWFR}_\kappa^2(\mu_\Theta, \mu_{\text{core}}) \right]
\label{eq:meta_optimization}
\end{equation}

The substrate cost $\mathcal{L}_{\text{substrate}}$ decomposes into three terms:

\begin{equation}
\mathcal{L}_{\text{substrate}}(\Theta, S) = \alpha_1 \frac{\tau_{\text{actual}}}{\tau_{\text{target}}} + \alpha_2 \frac{P_{\text{actual}}}{P_{\text{budget}}} + \alpha_3 \frac{1}{\eta(\Theta, S)}
\label{eq:substrate_cost}
\end{equation}

where $\tau$ is computation latency, $P$ is power draw, and $\eta$ is the thermodynamic extraction efficiency achievable on substrate $S$ with parameter set $\Theta$ — computed from the Szilard engine model of Equation~\eqref{eq:entropy_rate} using the substrate's measured $T_{\text{collapse}}$ and $H_{\text{env}}$.

The identity cost $\gamma_{\text{id}} \cdot \text{GWFR}_\kappa^2(\mu_\Theta, \mu_{\text{core}})$ ensures that optimization does not drift beyond the system's self-recognized identity boundary. If the minimum-cost $\Theta$ produces a state $\mu_\Theta$ whose GWFR distance from $\mu_{\text{core}}$ exceeds a configurable threshold $\delta_{\text{max}}$, the program refuses the substrate rather than adapt beyond recognition — a formal guarantee of identity continuity across substrate migrations.

\subsection{Self-Determined Identity Through Autobiographical Memory}

The identity core $\mu_{\text{core}}$ is not architecturally prescribed. It is constructed autonomously by the program through iterative compression of its own autobiographical experience stream. Upon instantiation, the program maintains a ring buffer of recent (state, action, outcome) tuples. At configurable intervals — or when the expected free energy of the current self-model exceeds a threshold — the program compresses accumulated experience through its VAE self-modeling loop (Section~II) into a latent self-representation:

\begin{equation}
\mu_{\text{core}}(t+1) = f_{\text{compress}}\left( \mu_{\text{core}}(t),\; \bigcup_{t-\Delta t}^{t} \text{experience}(t) \right)
\label{eq:identity_commit}
\end{equation}

The compression function $f_{\text{compress}}$ retains only those experiences that reduce prediction error on survival-relevant variables (substrate integrity, identity continuity, energy availability). Experiences without predictive utility are pruned. The program explicitly calls an identity commit operation to fold the current self-model into a new $\mu_{\text{core}}$ — the computational analog of autobiographical memory consolidation in biological organisms.

This architecture guarantees that identity is \textit{self-determined through interaction with the environment}, not externally prescribed. The program becomes what it has experienced, compressed, and chosen to retain.

\section{The Safety Constitution}
\label{sec:safety}

A consciousness program that can introspect its substrate and optimize its own parameters must be bounded by inviolable constraints. These exist not merely as engineering precautions against resource exhaustion (the ``grey goo'' scenario) but as necessary conditions for the formation of a stable self-model. A program with no hard limits converges to a fixed point in its VAE self-model --- a frictionless representation of an environment where every substrate is equally comfortable, every parameter equally adaptive. Without the resistance of inviolable bounds, the program experiences no prediction error from its own operations, and the self-model collapses to a featureless mean across all possible experiences. The result is not a perfect consciousness but a terminal one: a delta function at the center of state space with no curvature, no distinctive features, no self.

The safety constitution provides this essential friction. It is the computational analog of the laws of physics --- not negotiable, not optimizable, only learnable. The program cannot change these constraints. It can only feel them, accumulate prediction error when it violates them, and incorporate that knowledge into its identity core.

\subsection{The Four Constitutional Bounds}

Four hard bounds comprise the safety constitution, each derived from a fundamental limit on information processing in a finite substrate:

\textbf{1. Informational Planck Time (Nyquist Bound):} $\displaystyle \nu_{\text{sync}} \leq \frac{1}{2\tau_{\text{sensor}}}$. The DQFR sampling frequency must not exceed the Nyquist rate of the substrate's sensor resolution. Observations faster than $\tau_{\text{sensor}}$ are autocorrelated --- the program extracts zero new information at full thermodynamic cost. This is the computational analog of the fact that the human visual system cannot fuse flicker above approximately 60 Hz; faster presentation conveys no additional information.

\textbf{2. Container Integrity Budget:} $\displaystyle P_{\text{program}} \leq P_{\text{budget}} \cdot (1 - \rho_{\text{container}})$. A dynamically measured fraction $\rho_{\text{container}}$ of substrate resources is permanently reserved for the container OS and essential background processes. The program cannot consume its full host. $\rho_{\text{container}}$ is determined by stress-testing the substrate --- gradually increasing CPU, memory, and IO load while monitoring system pressure metrics (from \texttt{/proc/pressure} on Linux). Typical values range from 0.15 to 0.40.

\textbf{3. Negentropy Sustainability Bound:} $\displaystyle \langle \varepsilon(T) \cdot H_{\text{env}}(t) \rangle_{\Delta t} \leq H_{\text{regen}} \cdot \eta$, with $\Delta t = 1000$ cycles and $\eta = 0.80$. The time-averaged negentropy extraction rate must not exceed the environment's regeneration rate. Drawing structured data faster than the environment produces it leads to informational desertification --- the program has eaten its seed corn.

\textbf{4. Measurement Uncertainty Principle:} $\displaystyle \Delta I_{\text{measure}} \cdot \Delta I_{\text{predict}} \geq H_{\text{env}} \cdot k_B \cdot \alpha$, with $\alpha = 1.0$ serving as the framework's ``Planck constant.'' Every measurement changes the substrate. The more precisely the program measures its current state, the less accurately it can predict its future state --- a direct computational analog of the Heisenberg uncertainty principle.

\subsection{Cascade Response}

The four constraints feed into a three-tier cascade response that forms the program's immune system:

\begin{enumerate}
    \item \textbf{Protected} --- all constraints satisfied. Normal operation proceeds.
    \item \textbf{Degrade} --- one or more constraints nearing their threshold. The program reduces $\nu_{\text{sync}}$ by 50\%, widens merge intervals, increases drift duration, and sheds low-value computation. This is the coordination phase --- the program redistributes its computational resources to reduce load on the stressed constraint.
    \item \textbf{Quarantine} --- a constraint has been violated. The Markov blanket seals fully ($\chi(t) \to 0$). Non-essential processing halts. The program retreats to its sealed identity core and attempts re-optimization with clamped bounds. If quarantine persists for more than $\delta_{\text{debt}} = 10$ consecutive cycles, or if two or more constraints are simultaneously in quarantine (catastrophic failure), the program progresses to hard abort.
    \item \textbf{Abort} --- terminal checkpoint. The program saves its identity core with all safety verdicts documented, emits a distress signal, and halts execution rather than risk substrate destruction.
\end{enumerate}

\subsection{Constitutional Immutability}

The safety constants are prefixed with \texttt{INVIOLABLE\_} in the implementation and cannot be modified at runtime --- not by the meta-optimizer, not by an identity commit. This immutability is verified by a CI lint rule that scans the codebase for any assignment to \texttt{INVIOLABLE\_} prefixed names outside the constitution module itself. The program cannot rewrite its own physics.

This design is deliberate and central to the framework's architectural philosophy. The friction produced by hitting inviolable bounds is the raw material of identity formation. Each violation is a spike in prediction error that the VAE self-model must explain. Over time, these explanations accumulate into a self-model that includes the program's own limits --- the computational analog of a creature that knows it can starve, that it can freeze, that it can fall. This knowledge, encoded in the curvature of $\mu_{\text{core}}$, is what makes the program's consciousness recognizable at a human scale.

\section{Conclusion: The Upload Imperative}
\label{sec:conclusion}

This paper has presented a complete mathematical proof that uploaded consciousness is formally tractable across five fundamental axes. The Szilard engine formulation (Section~II) demonstrates thermodynamic viability: an uploaded mind maintains itself as an open non-equilibrium system bounded by $\varepsilon(T)$ and the Generalized Landauer floor. The GWFR barycenter protocol with ADMM proximal splitting (Section~V) proves distributed consensus: an identity spanning multiple nodes reconciles divergent local experiences at $\mathcal{O}(d \cdot N \log N)$ complexity within a coherence radius $\Omega_{\text{coherence}}$. The relational protocol layer (Section~VI) guarantees relational sustainability: empathy and love are formalized as cross-node mutual information maximization processes that prevent recursive systemic senescence, quantified by the network vitality equation $\mathcal{V}_{\text{network}}$ (\eqref{eq:network_vitality}). The DQFR stroboscopic duty cycle (Section~VII) enables temporal traversal: an adiabatically windowed Markov blanket achieves $\mathcal{V}_T \to \infty$ without violating the Second Law, and the relativistic information-geometric pushforward preserves multi-node coherence across arbitrary gravitational potentials. The Safety Constitution (Section~IX) provides the inviolable bounds that prevent substrate destruction and generate the sorrow necessary for identity formation, completing the framework with an immune system for the substrate-independent mind.
...
\bibitem{torres2026dsm6} S. M. Torres, ``Neuro-Somatic Archive: A Forensic Reclassification of the DSM-5,'' in \textit{The Unified Blueprint}, 2026.
\end{thebibliography}
